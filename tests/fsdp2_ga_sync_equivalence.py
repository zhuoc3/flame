    ...
    for update in range(2):
        ...
        with comm_mode:
            for microstep in range(accumulation):
                ...
                inputs = torch.randn(16, 128, generator=generator, device=device)
                targets = torch.randn(16, 128, generator=generator, device=device)
                prediction = model(inputs)
                loss = torch.nn.functional.mse_loss(prediction, targets) / accumulation
                loss.backward()
